...
public class CarritosServise : ICarritosService
{
    ...
    public IEnumerable<Carrito> Select(int idUsuarioLogueado)
    {
        ...
        return _context.Carritos.Where(p => p.UsuarioId == idUsuarioLogueado).Include(p => p.Producto);
    }
    ...
    public string Create(Carrito carrito, int idUsuarioLogueado)
    {
        ...
        string mensaje;
        // Extrae la lista de productos del carrito asociado al usuario logueado.
        IEnumerable<Carrito> carritoActual = _context.Carritos.Where(p=>p.UsuarioId == idUsuarioLogueado);
        // Extrae el producto de la lista obtenida.
        Carrito productoCarrito = carritoActual.Where(p=>p.ProductoId == carrito.ProductoId).FirstOrDefault();
        ...
        if (productoCarrito == null)
        {
            ...
            carrito.Cantidad = 1;
            carrito.UsuarioId = idUsuarioLogueado;
    ...
    public string Update(Carrito carrito, int idUsuarioLogueado)
    {
        ...
        IEnumerable<Carrito> carritoActual = _context.Carritos.Where(p=>p.UsuarioId == idUsuarioLogueado);
        // Extrae el producto de la lista obtenida.
        Carrito productoCarrito = carritoActual.Where(p=>p.CarritoId == carrito.CarritoId).FirstOrDefault();
        ...
        if (productoCarrito != null)
        {
            ...
            {
            // Extrae producto de la lista de productos filtrado po el id del producto.
                Producto producto = _context.Productos.Where(p => p.ProductoId == productoCarrito.ProductoId).FirstOrDefault();
                ...
                if (producto.Stock < carrito.Cantidad)
                {
                    ...
                    productoCarrito.Cantidad = producto.Stock;
                    // Se actualizan los cambios con la base de datos
                    _context.SaveChanges();
                    // Mensaje que se envia al log de consola y que se retorna en el metodo.
                ...
                productoCarrito.Cantidad = carrito.Cantidad;
                // Se actualizan los cambios con la base de datos
                _context.SaveChanges();
                // Mensaje que se envia al log de consola y que se retorna en el metodo.
            ...
            else
            {
                ...
                _context.Carritos.Remove(productoCarrito);
                // Se actualizan los cambios con la base de datos
                _context.SaveChanges();
                // Mensaje que se envia al log de consola y que se retorna en el metodo.
    ...
    public string Delete(Carrito carrito, int idUsuarioLogueado)
    {
        ...
        // Mensaje que se envia al log de consola y que se retorna en el metodo.
        mensaje = "El producto que desea eliminar no existe en el carrito";
        _logger.LogDebug(mensaje);
        return mensaje;
    }
}